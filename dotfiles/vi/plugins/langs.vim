...
augroup event_golang
  ...
  " enable/disable updating dependencies with |:GoInstallBinaries|.
  " let g:go_get_update = 1
  ...
  " don't use <Plug> go-run command, it is better to run with go run in term which will close by clicking any key
  " autocmd Filetype go nmap <silent><Leader>r :<C-u>split term://go run %<CR>
  "
  ...
  " go-navigate -----------------------------------------------
  " vim-go overrides the built in Vim shortcuts gd and ctrl-] as well.
  " autocmd FileType go nmap gd <Plug>(go-def)
  " autocmd FileType go nmap <C-]> <Plug>(go-def)
  " go back where call :GooDef(gd or <c-]) even had move around cursor
  autocmd FileType go nmap <leader>gb <Plug>(go-def-pop)
  " <C-[> is equivalent to ESC, which means can't map <C-[>
  ...
  " invode fzf to locate type and function declarations (current file or cross files)
  autocmd FileType go nmap <leader>f :<C-u>GoDecls<cr>
  autocmd FileType go nmap <leader>fd :<C-u>GoDeclsDir<cr>
  " vim-go provides(overrides) two motion objects to move between functions
  ...
  " To see the files that make a package
  " :GoFiles
  "
  " For showing the dependencies of a file
  " :GoDeps
  " info about the err on cursor
  " :GoWhicherrs

  " info about the channel on selected line
  " :GoChannelPeers

  " info about relation between functions
  " :GoCallees
  " :GoCallers
  " :GoCallstack
  ...
  " Extract function by :'<,'>GoFreevars in visual mode
  " :GoFreevars

  " Code generation-----------------------------------------
  " :GoGenerate command that calls go generate under the hood.
  " :GoGenerate

  " Method stubs implementing an interface. Put your cursor on top of T and type :GoImpl.
  " or invode it from everywhere like :GoImpl b *B fmt.Stringer
  " :GoImpl
